...
def main():
    ...
    tl.setParam("tcname", "TestCase6")
    tl.setParam("description", "This is a test case description")
    tl.setParam("fname", "112233 Example test feature")
    tl.setParam("coverage", 30)
    jsonstring = tl.to_JSON()

    # upload to db
    target_url = 'http://localhost/nliteme/upload.php';
    uploadtype = 'testcases'
    print(jsonstring)
    buf = StringIO()
    try:
        c = pycurl.Curl()
